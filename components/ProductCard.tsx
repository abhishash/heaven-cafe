    ...
    <Link href={`/product/${product.url}`}>
      <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden cursor-pointer h-full flex flex-col">
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          <p className="text-gray-600 text-sm line-clamp-2 mt-1 flex-1">{product.name}</p>

          <div className="flex items-center justify-between mt-4">
            <span className="text-2xl font-bold text-orange-600">${ formatPrice(parseInt(product.price) , "INR")}</span>
